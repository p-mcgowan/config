...
class CustomCloseUnmodifiedCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        window = self.view.window()
        window.run_command("close_unmodified", { "group": -1, "index": -1 })

class CustomCloseUnmodifiedToRightCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        window = self.view.window()
        window.run_command("close_unmodified_to_right_by_index", { "group": -1, "index": -1 })
