...
class CustomSortLinesCommand(sublime_plugin.TextCommand):
    def run(self, edit, case_sensitive=False, reverse=False, remove_duplicates=False, numeric=False):
        view = self.view

        if numeric:
            permute_selection(numeric_sort, view, edit)
        else:
            if case_sensitive:
...
class CustomSortSelectionCommand(sublime_plugin.TextCommand):
    def run(self, edit, case_sensitive=False, reverse=False, remove_duplicates=False, numeric=False):
        view = self.view
...
class CustomPermuteLinesCommand(sublime_plugin.TextCommand):
    def run(self, edit, operation='shuffle'):
        permute_lines(permute_funcs[operation], self.view, edit)


class CustomPermuteSelectionCommand(sublime_plugin.TextCommand):
    def run(self, edit, operation='shuffle'):
        view = self.view
